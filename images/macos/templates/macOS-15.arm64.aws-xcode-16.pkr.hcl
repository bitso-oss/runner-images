...
build {
  ...
  provisioner "file" {
    ...
    source      = "${path.root}/../toolsets/toolset-15-16.json"
  }

  provisioner "shell" {
    execute_command = "sudo sh -c '{{ .Vars }} {{ .Path }}'"
    inline          = [
      "rm -rf ${local.image_folder}/software-report && mv ${local.image_folder}/docs-gen ${local.image_folder}/software-report",
      "mkdir -p ~/utils",
  ...
  provisioner "shell" {
    inline = [
      "echo '=========================================='",
      "echo 'FINALIZING IMAGE CONSTRUCTION'",
      "echo '=========================================='",
      "echo 'Executing final configuration steps...'",
      "echo '=========================================='"
    ]
  }
